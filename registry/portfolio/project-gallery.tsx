    ...
    >
      {projects.map((project) => (
        <a
          key={project.title}
          href={project.url ?? "#"}
          className="group block overflow-hidden rounded-xl ring-1 ring-foreground/10"
        >
          <AspectRatio ratio={4 / 3} className="flex flex-col justify-end p-3">
            <div
              className={cn(
            ...
            />
            <Badge className="absolute top-3 right-3 size-7 rounded-full border-white/10 bg-white/20 p-0 text-white opacity-0 backdrop-blur transition-opacity group-hover:opacity-100">
